...
def main():
    ...
    model = RobertaModel.from_pretrained("/home/linzexu/huggingface/cocosoda")
    ...
    '''
    list_all = ["C", "C#", "C++", "D", "Haskell", "Java", "JavaScript", "PHP", "Python", "Rust"]
    examples = {}
    for i in list_all:
        examples[i] = []
        lang_dir = os.path.join(args.train_data_file, i)
        lang_files = os.listdir(lang_dir)
        for lang_file in lang_files:
            with open(os.path.join(lang_dir, lang_file), 'r') as f:
                code = ""
                ...
                # print(source_ids)
                source_ids+=[tokenizer.pad_token_id]*padding_length
                print(len(source_ids), len(source_tokens), i)
                    # inputFeatures = InputFeatures(source_tokens,source_ids,i)
            examples[i].append(inputFeatures)
